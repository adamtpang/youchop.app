...
window.VITALS = {
  ...
  products: [
    { name: "Channel → Text — search a channel, extract its transcripts for LLMs (/extract)", status: "Live" }, // FREE stack: YT_DATA_API_KEY (search/listing) + APIFY_TOKEN (transcripts)
  ...
  decisions: [
    ...
    { date: "2026-07-02", decision: "Channel → Text runs on a FREE backend stack, not a paid API", why: "Transcript scraping's real constraint is that YouTube blocks datacenter IPs (Vercel/AWS/GCP) + PoToken, so direct free scrapers (youtube-transcript-api) die in production; that's what paid APIs charge for (residential proxies). Chosen free stack: (1) YouTube Data API v3 for channel SEARCH + video listing — genuinely free, 10k units/day, reliable; but its captions.download only works for videos you OWN, so it can't fetch arbitrary transcripts. (2) Apify free tier ($5/mo credits) for transcripts — runs the scrape on Apify's infra so it survives the IP block; ~$0.007/video via karamelo/youtube-transcripts. Providers are pluggable (TRANSCRIPT_PROVIDER); youtube-transcript.io kept as an optional paid fallback. Both required keys are free.", revisit: "If Apify free credits run out at real volume, or if the Chrome extension ships (it can fetch transcripts client-side on the user's residential IP for free — the ultimately-robust path)." },
    { date: "2026-07-02", decision: "New wedge shipped: 'Channel → Text' — pull a whole channel's transcripts as copy-paste LLM context (/extract)", why: "Feeding YouTube into Claude/ChatGPT/Cursor is a sharper, more current pain than viewer chapters, and it reuses the same transcript infrastructure. It's a consumer-utility wedge with a clear job-to-be-done ('put a creator's brain in my context window'). Shipped as a first-class tool alongside chapters; not yet decided whether it becomes THE primary product. Uses youtube-transcript.io: transcripts endpoint (any paid tier) + channels endpoint (Plus $9.99/mo). Token stays server-side; mock mode ships so the flow demos without a key.", revisit: "If /extract usage clearly outpaces the chapters/creator funnel, promote it to the north-star product and re-orient the site around it. Decide after 2–3 weeks of real usage." },
